// Note: This is for browser environment, so we'll mock the exec functionality

// Mock exec functionality for browser environment
const execAsync = async (_command: string): Promise<{ stdout: string; stderr: string }> => {
  throw new Error('exec not available in browser environment');
};
...
export async function fetchWithRetry<T>(url: string, options: RequestOptions = {}): Promise<T> {
  try {
    ...
    try {
      ...
      console.log(`[fetchWithRetry] Executing curl command: ${curlCommand}`);
      ...
      if (stderr) {
        ...
        if (
          !stdout ||
          (stderr as string).toLowerCase().includes("error") ||
          (stderr as string).toLowerCase().includes("fail")
        ) {
      ...
      console.error(`[fetchWithRetry] Curl fallback also failed for ${url}: ${curlError.message}`);
